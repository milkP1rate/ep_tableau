...
// CommonJS
if (typeof (require) != 'undefined') { // deepscan-disable-line INSUFFICIENT_NULL_CHECK
...
exports.acePostWriteDomLineHTML = function (hook_name, args, cb) {
  ...
  if (node.children.length > 0) {
    ...
    for (var i = 1; i < node.children.length; i++) {
      let innerHTML = node.children[i].innerHTML;
...
exports.aceKeyEvent = function (hook, context) {
  ...
  try {
    ...
    if (Datatables.isFocused()) {
      ...
      var which = evt.which;
      if (isTypeForSpecialKey && keyCode == 9 && !(evt.metaKey || evt.ctrlKey)) {
      ...
      if (isTypeForSpecialKey && ((type == "keydown" && keyCode == Datatables.vars.JS_KEY_CODE_DEL) || keyCode == Datatables.vars.JS_KEY_CODE_BS || (String.fromCharCode(which).toLowerCase() == "h" && (evt.ctrlKey)))) {
...
var Datatables = function () {
  ...
  */
  ...
  dt.addTable = function (tableObj) {
    ...
    if (tableObj == null) {
      let editorInfo = this.context.editorInfo;
      let authors = editorInfo.ace_getAuthor();
    ...
    try {
      ...
      if (aboveOrBelow == 'addA') {
        ...
        if (isFirstRow = jsoTblProp.isFirstRow) {
          ...
          this.updateTblPropInAPool(-1, -1, jsoTblProp, rep.selStart);
        }
  ...
  dt.updateTableProperties = function (props) {
    ...
    if (props.tblWidth || props.tblHeight || props.tblBorderWidth || props.tblBorderColor || props.tblColWidth || props.tblSingleColBgColor || props.tblEvenRowBgColor || props.tblOddRowBgColor || props.tblColVAlign) {
      ...
      this.updateTablePropertiesHelper(props, rep.selStart, currTd);
    }
  };
  ...
  dt.addColumnAttr = function (start, tblJSONObj, tblProperties, attrName, attrValue, currTd) {
    ...
    if (typeof (colAttrs) == 'undefined' || colAttrs == null) {
      colAttrs = [];
    }
    if (colAttrs[currTd] == null) {
      colAttrs[currTd] = {};
  ...
  dt.updateTablePropertiesHelper = function (props, start, currTd) {
    ...
    try {
      ...
      if (props.tblCellFont || props.tblCellFontSize || props.tblCellBgColor || props.tblCellHeight || props.tblcellVAlign || props.tblCellBold || props.tblCellItalic || props.tblCellDecoration || props.tblCellVAlign || props.tblCellHAlign) {
        let tblProps = this.addCellAttr(start, tblJSONObj, tblProperties, props.attrName, props.attrValue);
        if (tblProps) {
      ...
      if (props.tblEvenRowBgColor || props.tblOddRowBgColor) {
        let tblProps = this.addRowAttr(tblJSONObj, tblProperties, props.attrName, props.attrValue);
        if (tblProps) {
          tblProperties = tblProps;
          update = true;
        }
      ...
      if (props.tblColWidth || props.tblSingleColBgColor || props.tblColVAlign) {
        let tblProps = this.addColumnAttr(start, tblJSONObj, tblProperties, props.attrName, props.attrValue, currTd);
        if (tblProps) {
          tblProperties = tblProps;
          update = true;
        }
  ...
  dt.insertTblRowBelow = function (numOfRows, table) {
    let rep = this.rep;
    let currLineText = rep.lines.atIndex(rep.selStart[0]).text;
    let payload;
